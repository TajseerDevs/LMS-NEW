...
const MatchInputs = ({questionObj , setQuestionObj , quizData , refetch}) => {
    ...
    const handleAddOption = () => {
        ...
        const newOption = {
          ...
          matchWith : `match with option ${options.length + 1}` ,
          isCorrect: true,  
          feedback    
        }
    ...
    const handleCreateNewQuestion = async () => {
        ...
        const updatedOptions = options.map(option => ({
            ...option,
            feedback: `${option.matchWith} is the correct match answer`
        }))
    ...
    const handleDeleteOption = (indexToDelete) => {
        ...
        const renumberedOptions = updatedOptions.map((option, index) => ({
          ...
          matchWith : `match with option ${index + 1}` ,
        }))
